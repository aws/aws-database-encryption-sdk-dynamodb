  ...
  DynamoDbEncryptionTransforms refines AbstractAwsCryptographyDbEncryptionSdkDynamoDbTransformsService
{
  ...
  method {:vcs_split_on_every_assert} DynamoDbEncryptionTransforms(config: AwsCryptographyDbEncryptionSdkDynamoDbTypes.DynamoDbTablesEncryptionConfig)
    ...
    // ensures config.tableEncryptionConfigs[SortedSets.ComputeSetToSequence(config.tableEncryptionConfigs.Keys)[i]].search.Some? ==> config.tableEncryptionConfigs[SortedSets.ComputeSetToSequence(config.tableEncryptionConfigs.Keys)[i]].search.value.versions[0].keyStore.ValidState()
              ...
              && DdbMiddlewareConfig.ValidConfig?(config)
  {
    ...
    while i < |tableNamesSeq|
      ...
      modifies if i < |tableNamesSeq| && config.tableEncryptionConfigs[tableNamesSeq[i]].search.Some? 
        then config.tableEncryptionConfigs[tableNamesSeq[i]].search.value.versions[0].keyStore.Modifies 
        else {}
    {
      ...
      // modifies if inputConfig.search.Some? then inputConfig.search.value.versions[0].keyStore.Modifies else {};
