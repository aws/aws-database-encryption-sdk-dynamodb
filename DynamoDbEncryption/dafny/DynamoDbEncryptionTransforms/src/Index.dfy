  ...
  DynamoDbEncryptionTransforms refines AbstractAwsCryptographyDbEncryptionSdkDynamoDbTransformsService
{
              ...
              && DdbMiddlewareConfig.ValidConfig?(config)
  {
    ...
    while i < |tableNamesSeq|
      ...
      modifies if i < |tableNamesSeq| && config.tableEncryptionConfigs[tableNamesSeq[i]].search.Some? then config.tableEncryptionConfigs[tableNamesSeq[i]].search.value.versions[0].keyStore.Modifies else {}
    {
      ...
      assert inputConfig.search.Some? ==> inputConfig.search.value.versions[0].keyStore.ValidState();
