...
import com.amazonaws.services.dynamodbv2.datamodeling.sdkv2.testing.FakeKMS;
...
public class DirectKmsMaterialProviderTest {
  ...
  public void simpleWithKmsEc3() throws GeneralSecurityException {
    ...
    assertEquals(
      Base64.encodeToString("Bar".getBytes(StandardCharsets.UTF_8)),
      kmsCtx.get("rk")
    );
    assertEquals("KmsTableName", kmsCtx.get("*aws-kms-table*"));
  ...
  public void extendedWithDerivedEncryptionKeyId() {
    ExtendedKmsMaterialsProvider prov = new ExtendedKmsMaterialsProvider(
      kms,
      keyId,
      "encryptionKeyId"
    );
    ...
    attrVals.put(
      "encryptionKeyId",
      AttributeValue.builder().s(customKeyId).build()
    );
  ...
  private static class ExtendedKmsMaterialsProvider
    ...
    protected String selectEncryptionKeyId(EncryptionContext context)
      ...
      if (
        ...
        throw DynamoDbException.create(
    ...
    protected void validateEncryptionKeyId(
      ...
      if (!customEncryptionKeyId.equals(encryptionKeyId)) {
        throw DynamoDbException.create(
          "encryption key ids do not match.",
          new Exception()
        );
      }
    }
    ...
    @Override
    protected DecryptResponse decrypt(
