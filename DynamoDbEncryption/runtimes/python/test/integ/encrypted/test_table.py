...
def test_GIVEN_items_WHEN_batch_write_and_get_THEN_round_trip_passes(
    ...
    with table.batch_writer() as batch_writer:
        batch_writer.delete_item({"partition_key": simple_item_dict["partition_key"], "sort_key": simple_item_dict["sort_key"]})
        batch_writer.delete_item({"partition_key": complex_item_dict["partition_key"], "sort_key": complex_item_dict["sort_key"]})
