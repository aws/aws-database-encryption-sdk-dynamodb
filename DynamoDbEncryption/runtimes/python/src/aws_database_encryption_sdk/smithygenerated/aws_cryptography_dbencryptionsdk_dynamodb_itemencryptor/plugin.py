...
from .config import Config, Plugin, smithy_config_to_dafny_config, DynamoDbItemEncryptorConfig
...
def set_config_impl(config: Config):
    ...
    if isinstance(config, DynamoDbItemEncryptorConfig):
        from aws_database_encryption_sdk.internaldafny.generated.DynamoDbItemEncryptor import default__
        config.dafnyImplInterface.impl = default__.DynamoDbItemEncryptor(smithy_config_to_dafny_config(config)).value
