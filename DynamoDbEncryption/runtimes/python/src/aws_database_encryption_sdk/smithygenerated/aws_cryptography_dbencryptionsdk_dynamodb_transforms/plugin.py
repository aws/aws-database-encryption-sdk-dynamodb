...
from .config import Config, Plugin, smithy_config_to_dafny_config, DynamoDbTablesEncryptionConfig
...
def set_config_impl(config: Config):
    ...
    if isinstance(config, DynamoDbTablesEncryptionConfig):
        from aws_database_encryption_sdk.internaldafny.generated.DynamoDbEncryptionTransforms import default__
        config.dafnyImplInterface.impl = default__.DynamoDbEncryptionTransforms(smithy_config_to_dafny_config(config)).value
