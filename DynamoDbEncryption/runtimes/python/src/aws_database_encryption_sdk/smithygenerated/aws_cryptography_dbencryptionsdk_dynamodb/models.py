...
class MultiKeyStore:
    ...
    def __eq__(self, other: Any) -> bool:
        if not isinstance(other, MultiKeyStore):
            return False
        attributes: list[str] = ['key_field_name','cache_ttl','cache',]
        return all(
...
class SingleKeyStore:
    ...
    def __eq__(self, other: Any) -> bool:
        if not isinstance(other, SingleKeyStore):
            return False
        attributes: list[str] = ['key_id','cache_ttl',]
        return all(
            getattr(self, a) == getattr(other, a)
            for a in attributes
        )
...
class CreateDynamoDbEncryptionBranchKeyIdSupplierInput:
    ddb_key_branch_key_id_supplier: 'aws_database_encryption_sdk.smithygenerated.aws_cryptography_dbencryptionsdk_dynamodb.references.DynamoDbKeyBranchKeyIdSupplier'
    def __init__(
        self,
        *,
        ddb_key_branch_key_id_supplier: 'aws_database_encryption_sdk.smithygenerated.aws_cryptography_dbencryptionsdk_dynamodb.references.DynamoDbKeyBranchKeyIdSupplier',
...
class LegacyOverride:
    policy: str
    encryptor: 'aws_database_encryption_sdk.smithygenerated.aws_cryptography_dbencryptionsdk_dynamodb.references.LegacyDynamoDbEncryptor'
...
class DynamoDbTableEncryptionConfig:
    ...
    cmm: Optional['aws_cryptographic_materialproviders.smithygenerated.aws_cryptography_materialproviders.references.CryptographicMaterialsManager']
    ...
    def __init__(
        ...
        *,
        ...
        cmm: Optional['aws_cryptographic_materialproviders.smithygenerated.aws_cryptography_materialproviders.references.CryptographicMaterialsManager'] = None,
...
def _dynamo_db_table_encryption_config_list_as_dict(given: dict[str, DynamoDbTableEncryptionConfig]) -> Dict[str, Any]:
    return {k: v.as_dict() for k, v in given.items()}

def _dynamo_db_table_encryption_config_list_from_dict(given: Dict[str, Any]) -> dict[str, DynamoDbTableEncryptionConfig]:
    return {k: DynamoDbTableEncryptionConfig.from_dict(v) for k, v in given.items()}

def _beacon_version_list_as_dict(given: list[BeaconVersion]) -> List[Any]:
    return [v.as_dict() for v in given]

def _beacon_version_list_from_dict(given: List[Any]) -> list[BeaconVersion]:
    return [BeaconVersion.from_dict(v) for v in given]

def _signed_parts_list_as_dict(given: list[SignedPart]) -> List[Any]:
    return [v.as_dict() for v in given]

def _signed_parts_list_from_dict(given: List[Any]) -> list[SignedPart]:
    return [SignedPart.from_dict(v) for v in given]

def _encrypted_parts_list_as_dict(given: list[EncryptedPart]) -> List[Any]:
    return [v.as_dict() for v in given]

def _encrypted_parts_list_from_dict(given: List[Any]) -> list[EncryptedPart]:
    return [EncryptedPart.from_dict(v) for v in given]

def _virtual_field_list_as_dict(given: list[VirtualField]) -> List[Any]:
    return [v.as_dict() for v in given]

def _virtual_field_list_from_dict(given: List[Any]) -> list[VirtualField]:
    return [VirtualField.from_dict(v) for v in given]

def _virtual_part_list_as_dict(given: list[VirtualPart]) -> List[Any]:
    return [v.as_dict() for v in given]

def _virtual_part_list_from_dict(given: List[Any]) -> list[VirtualPart]:
    return [VirtualPart.from_dict(v) for v in given]

def _virtual_transform_list_as_dict(given: list[VirtualTransform]) -> List[Any]:
    return [v.as_dict() for v in given]

def _virtual_transform_list_from_dict(given: List[Any]) -> list[VirtualTransform]:
    return [VirtualTransform.from_dict(v) for v in given]

def _compound_beacon_list_as_dict(given: list[CompoundBeacon]) -> List[Any]:
    return [v.as_dict() for v in given]

def _compound_beacon_list_from_dict(given: List[Any]) -> list[CompoundBeacon]:
    return [CompoundBeacon.from_dict(v) for v in given]

def _constructor_list_as_dict(given: list[Constructor]) -> List[Any]:
    return [v.as_dict() for v in given]

def _constructor_list_from_dict(given: List[Any]) -> list[Constructor]:
    return [Constructor.from_dict(v) for v in given]

def _constructor_part_list_as_dict(given: list[ConstructorPart]) -> List[Any]:
    return [v.as_dict() for v in given]

def _constructor_part_list_from_dict(given: List[Any]) -> list[ConstructorPart]:
    return [ConstructorPart.from_dict(v) for v in given]

def _standard_beacon_list_as_dict(given: list[StandardBeacon]) -> List[Any]:
    return [v.as_dict() for v in given]

def _standard_beacon_list_from_dict(given: List[Any]) -> list[StandardBeacon]:
    return [StandardBeacon.from_dict(v) for v in given]
