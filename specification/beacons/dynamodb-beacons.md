...
A full beacon configuration MUST contain a list of beacon version configurations.

A beacon version configurations MUST contain
 
 * The [Key Indicator](./beacons.md#key-indicator) used to generate the data keys to perform the HMAC for the beacons.
 * A list of [beacon configuration](./beacons.md#scan-beacon-configuration)
...
been added in the previous point, this operation MUST fail
 * If any Global or Local Secondary Index is being created that references
an encrypted field in its Key Configuration,
 * * if that field does not have a beacon configured, this operation MUST fail.
...
this operation MUST fail; otherwise,
 ...
 * This operation MUST take as input a GetItemInput object.
 * This operation MUST return the unaltered input object.

### transformBatchGetItemInput
 * This operation MUST take as input a BatchGetItem Input object.
 * This operation MUST return the unaltered input object.

### transformTransactGetItemsInput
 * This operation MUST take as input a TransactGetItemsInput object.
 * This operation MUST return the unaltered input object.

### transformExecuteStatementInput
 * This operation MUST take as input an ExecuteStatementInput object.
 * This operation MUST return an ExecuteStatementInput object.
 * This operation MUST fail

### transformBatchExecuteStatementInput
 * This operation MUST take as input an BatchExecuteStatementInput object.
 * This operation MUST return an BatchExecuteStatementInput object.
 * This operation MUST fail

### transformExecuteTransactionInput
 * This operation MUST take as input an ExecuteTransactionInput object.
 * This operation MUST return an ExecuteTransactionInput object.
 * This operation MUST fail

### Expression Transformation
 * this is a virtual entry point, describing the modification of a
Filter Expression, Condition Expression or Key Condition Expression,
 ...
 * This operation MUST fail if any beaconed field is used with a document path.
 ...
 * If the input object's `Filter Expression` must be modified according to `Expression Transformation` above.
 ...
 * If the input object's keyConditionExpression must be modified according to `Expression Transformation` above.
 ...
 * This operation must extract `Number` from the input QueryInput by calling GetQueryNumber.
 * This operation must extract `Total` from the input QueryInput by calling GetTotalQueries.
 * If `Number` is greater than `Total` this operation must fail
 * If `Number` equals `Total` and the input QueryOutput has no lastEvaluatedkey, 
the output must have no lastEvaluatedkey
 * If `Number` is less than `Total` and the input QueryOutput has no lastEvaluatedkey, 
the output must have a lastEvaluatedkey equal to `{"#:N:#": {}}` where N is equal to `Number`
 * If `Number` is less than `Total` and the input QueryOutput has a lastEvaluatedkey,
the name of the first attribute of the lastEvaluatedkey must be prefixed with `#:N:#` 
where N is equal to `Number`
