...
Thus one can optionally configure a list of split lengths
such that the first string in the list generated by the split uses
the first [beacon length](#beacon-length) in [split lengths](#split-lengths).

If [split lengths](#split-lengths) are used, then the first length must be equal
 ...
 * An OPTIONAL `inner` character to designate an [inner prefix](#inner-prefix)
