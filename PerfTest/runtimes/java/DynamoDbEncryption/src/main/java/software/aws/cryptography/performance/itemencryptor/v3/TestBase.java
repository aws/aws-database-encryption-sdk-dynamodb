...
public abstract class TestBase {
  ...
  protected Map<String, AttributeValue.Builder> plainTextAttribute =
    new HashMap<>();
  ...
  protected Map<String, CryptoAction> getAttributeActions(
    Map<String, AttributeValue.Builder> record
  ) {
  ...
  public void setup() throws Exception {
    final JsonNode plainTextJson = mapper.readTree(
      getClass().getClassLoader().getResourceAsStream(plainTextFile)
    );
    plainTextJson
      .properties()
      .forEach(entry ->
        plainTextAttribute.put(
          entry.getKey(),
          mapper.convertValue(
            entry.getValue(),
  ...
  public Map<String, AttributeValue> encrypt() throws Exception {
    ...
    originalItem.put(
      PARTITION_ATTRIBUTE,
      AttributeValue.builder().s(TEST_PK).build()
    );
    originalItem.put(
      SORT_ATTRIBUTE,
      AttributeValue.builder().n(SORT_NUMBER).build()
    );
    plainTextAttribute.forEach((key, value) ->
      originalItem.put(key, value.build())
    );
  ...
  public Map<String, AttributeValue> decrypt() {
    final Map<String, AttributeValue> decryptedItem = dynamoDbItemEncryptor
      .DecryptItem(
        DecryptItemInput.builder().encryptedItem(encryptedAttributes).build()
      )
      .plaintextItem();
  ...
  public void writeSizeToFile() throws Exception {
    ...
    itemBeforeEncrypt.forEach((key, value) ->
      originalObject.put(key, value.toBuilder())
    );
    encryptedAttributes.forEach((key, value) ->
      encryptedObject.put(key, value.toBuilder())
    );
    ...
    int lengthBeforeEncrypt = mapper
      .writeValueAsString(originalObject)
      .length();
    int lengthAfterEncrypt = mapper
      .writeValueAsString(encryptedObject)
      .length();
