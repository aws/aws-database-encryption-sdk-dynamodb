...
public class MostRecentKeyProviderTest extends TestBase {
    public static final String TABLE_MOSTRECENT_EXAMPLE_KEYS = "v2MostRecentKeyProviderPerfTestKeys";
    @Override
    protected EncryptionMaterialsProvider createMasterKeyProvider() {
        final AWSKMS kms = AWSKMSClientBuilder.standard().withRegion(Regions.DEFAULT_REGION).build();
        AmazonDynamoDB ddb = AmazonDynamoDBClientBuilder.standard().withRegion(Regions.DEFAULT_REGION).build();

        final DirectKmsMaterialProvider kmsProv = new DirectKmsMaterialProvider(kms, KMS_KEY_ARN);
        final DynamoDBEncryptor keyEncryptor = DynamoDBEncryptor.getInstance(kmsProv);
        final MetaStore metaStore = new MetaStore(ddb, TABLE_MOSTRECENT_EXAMPLE_KEYS, keyEncryptor);

        final CachingMostRecentProvider cmp =
                new CachingMostRecentProvider(metaStore, "testMaterial", 60_000);
        return cmp;
    }
