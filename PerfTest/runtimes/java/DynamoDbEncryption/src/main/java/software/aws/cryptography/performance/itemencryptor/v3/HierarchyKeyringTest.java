...
public class HierarchyKeyringTest extends TestBase {
  ...
  private static final String TEST_DDB_TABLE_NAME =
  ...
  protected IKeyring createKeyring() {
    ...
    final KeyStoreConfig keyStoreConfig = KeyStoreConfig
      .builder()
      .ddbTableName(TEST_KEYSTORE_NAME)
      .logicalKeyStoreName(TEST_KEYSTORE_NAME)
      .kmsConfiguration(
        KMSConfiguration.builder().kmsKeyArn(KMS_KEY_ARN).build()
      )
      .kmsClient(kmsClient)
      ...
      .CreateDynamoDbEncryptionBranchKeyIdSupplier(
        CreateDynamoDbEncryptionBranchKeyIdSupplierInput
          ...
          .ddbKeyBranchKeyIdSupplier(getBranchKeyIdFromDdbKeyInput -> {
            String tenantKeyId = getBranchKeyIdFromDdbKeyInput
              .ddbKey()
              .get(PARTITION_ATTRIBUTE)
              .s();
            if (tenantKeyId.equals(TEST_PK)) {
              return GetBranchKeyIdFromDdbKeyOutput
                .builder()
                .branchKeyId(tenant1BranchKey)
                .build();
            }
            return GetBranchKeyIdFromDdbKeyOutput
              .builder()
              .branchKeyId(tenant2BranchKey)
              .build();
          })
          .build()
      )
