...
public class DBESDKBenchmark {
  ...
  public List<BenchmarkResult> runAllBenchmarks() throws Exception {
    ...
    List<Integer> allDataSizes = Stream
      .of(
    ...
    List<String> allowedTestTypes = config.getQuickConfig() != null
      ? config.getQuickConfig().getTestTypes()
      : null;
    ...
    System.out.printf(
      "Benchmark suite completed. Total results: %d%n",
      results.size()
    );
    return results;
  }
   ...
   */
  private void runThroughputTests(List<Integer> dataSizes, int iterations)
    throws Exception {
    ...
    for (int dataSize : dataSizes) {
      ...
      if (result != null) {
        results.add(result);
        System.out.printf(
          "Throughput test completed: %.2f ops/sec%n",
   ...
   */
  private BenchmarkResult runThroughputTest(int dataSize, int iterations)
    throws Exception {
    System.out.printf(
      "Running throughput test - Size: %d bytes, Iterations: %d%n",
      dataSize,
      iterations
    );
    ...
    double totalDuration =
      Utils.nanosToMillis(System.nanoTime() - startTime) / 1000.0; // Convert to seconds

    // Calculate metrics
    double[] endToEndArray = endToEndLatencies
      .stream()
    ...
    System.out.printf(
      "Throughput test completed - Ops/sec: %.2f, MB/sec: %.2f%n",
      result.getOpsPerSecond(),
      result.getBytesPerSecond() / (1024 * 1024)
    );
  ...
  private void runMemoryTests(List<Integer> dataSizes) throws Exception {
    ...
    for (int dataSize : dataSizes) {
      ...
      if (result != null) {
        results.add(result);
        System.out.printf(
          "Memory test completed: %.2f MB peak%n",
          result.getPeakMemoryMb()
        );
      }
    }
  }
  ...
  private BenchmarkResult runMemoryTest(int dataSize) throws Exception {
    System.out.printf(
      "Running memory test - Size: %d bytes (%d iterations, continuous sampling)%n",
      dataSize,
    ...
    result.setMemoryEfficiencyRatio(
      memoryResults.peakMemoryMb > 0
        ? dataSize / (memoryResults.peakMemoryMb * 1024 * 1024)
        : 0.0
    );
    result.setTimestamp(Utils.getCurrentTimestamp());
    result.setJavaVersion(Utils.getJavaVersion());
  ...
  private MemoryResults sampleMemoryDuringOperations(byte[] data) {
    ...
    double overallAvgMemory = avgMemoryValues.isEmpty()
      ? 0.0
      : avgMemoryValues
        .stream()
        .mapToDouble(Double::doubleValue)
    ...
    System.out.println("\nMemory Summary:");
    System.out.printf(
      "- Absolute Peak Heap: %.2f MB (across all runs)%n",
      peakMemoryDelta
    );
    System.out.printf(
      "- Average Heap: %.2f MB (across all runs)%n",
      overallAvgMemory
    );
    System.out.printf(
      "- Total Allocations: %.2f MB (max across all runs)%n",
      peakAllocations
    );
    System.out.flush();
  ...
  private IterationResult runSingleMemoryIteration(byte[] data, int iteration) {
    ...
    try {
      ...
      System.out.printf(
        "Memory test iteration %d failed: %s%n",
        iteration,
        e.getMessage()
      );
  ...
  private IterationResult calculateIterationMetrics(
    ...
    System.out.printf(
      "=== Iteration %d === Peak Heap: %.2f MB, Total Allocs: %.2f MB, " +
      ...
      for (int concurrency : concurrencyLevels) {
        if (concurrency > 1) { // Skip single-threaded
          ...
          if (result != null) {
            results.add(result);
            System.out.printf(
              "Concurrent test completed: %.2f ops/sec @ %d threads%n",
              result.getOpsPerSecond(),
              concurrency
            );
          }
   ...
   */
  private BenchmarkResult runConcurrentTest(
    int dataSize,
    int concurrency,
    int iterationsPerWorker
  ) throws Exception {
    System.out.printf(
      "Running concurrent test - Size: %d bytes, Concurrency: %d%n",
      dataSize,
      concurrency
    );
          ...
          () -> {
            for (int j = 0; j < iterationsPerWorker; j++) {
              try {
                ...
                runItemEncryptorCycle(data);
                double iterMs = Utils.nanosToMillis(
                  System.nanoTime() - iterStart
                );
                allTimes.add(iterMs);
              } catch (Exception e) {
                throw new RuntimeException(
                  String.format(
                    "Worker %d iteration %d failed: %s",
                    workerId,
                    j,
                    e.getMessage()
                  ),
    ...
    System.out.printf(
      "Concurrent test completed - Ops/sec: %.2f, Avg latency: %.2f ms%n",
      result.getOpsPerSecond(),
      result.getEndToEndLatencyMs()
    );
  ...
  private LatencyResult runItemEncryptorCycle(byte[] data) throws Exception {
    ...
    item.put(
      "partition_key",
      AttributeValue.builder().s("benchmark-test").build()
    );
    ...
    nestedMap.put(
      "data",
      AttributeValue.builder().b(SdkBytes.fromByteArray(data)).build()
    );
    ...
    EncryptItemInput encryptInput = EncryptItemInput
      .builder()
      .plaintextItem(item)
      .build();
    ...
    DecryptItemInput decryptInput = DecryptItemInput
      .builder()
