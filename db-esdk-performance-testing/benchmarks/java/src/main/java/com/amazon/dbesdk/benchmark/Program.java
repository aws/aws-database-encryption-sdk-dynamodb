...
public class Program {
    ...
    private static CommandLineOptions parseArgs(String[] args) {
        ...
        options.addOption(Option.builder("c")
                ...
                .hasArg()
                .argName("file")
                .desc("Path to test configuration file (default: ../config/test-scenarios.yaml)")
                .build());
        ...
        options.addOption(Option.builder("o")
                ...
                .hasArg()
                .argName("file")
                .desc("Path to output results file (default: ../results/raw-data/java_results.json)")
                .build());
        ...
        try {
            ...
            String outputPath = cmd.getOptionValue("o", "../results/raw-data/java_results.json");
    ...
    private static void printUsage(Options options) {
        ...
        System.out.println("  java -jar db-esdk-benchmark.jar -c /path/to/config.yaml -o /path/to/results.json");
        System.out.println();
     ...
     */
    private static void printTestTypeSummary(List<BenchmarkResult> results, String testType) {
        List<BenchmarkResult> testResults = results.stream()
        ...
        if ("throughput".equals(testType)) {
            testResults.stream()
                    ...
                    .forEach(r -> System.out.printf("  %s: %.2f ops/sec, %.2f ms latency%n",
                            formatDataSize(r.getDataSize()), r.getOpsPerSecond(), r.getEndToEndLatencyMs()));
        } else if ("memory".equals(testType)) {
            testResults.stream()
                    .limit(3)
                    .forEach(r -> System.out.printf("  %s: %.2f MB peak, %.4f efficiency%n",
                            formatDataSize(r.getDataSize()), r.getPeakMemoryMb(), r.getMemoryEfficiencyRatio()));
        } else if ("concurrent".equals(testType)) {
            testResults.stream()
                    .limit(3)
                    .forEach(r -> System.out.printf("  %s (%d threads): %.2f ops/sec%n",
                            formatDataSize(r.getDataSize()), r.getConcurrency(), r.getOpsPerSecond()));
        }
    }
    ...
    public static class CommandLineOptions {
        ...
        public CommandLineOptions(String configPath, String outputPath, boolean quickTest) {
