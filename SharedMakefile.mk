...
DDBEC_ROOT := $(realpath $(dir $(realpath $(lastword $(MAKEFILE_LIST)))))
...
# So rather than use $(DDBEC_ROOT)% we use $(PROJECT_ROOT)/../%
# 
...
transpile_implementation:
	dafny \
		...
		-out $(OUT) \
		./src/Index.dfy \
		$(patsubst %, -library:$(DDBEC_ROOT)/%/src/Index.dfy, $(LIBRARIES))
...
transpile_dependencies:
	$(patsubst %, $(MAKE) -C $(DDBEC_ROOT)/% transpile_implementation_$(LANG);, $(LIBRARIES))
...
mvn_local_deploy_dependencies:
	$(patsubst %, $(MAKE) -C $(DDBEC_ROOT)/% mvn_local_deploy;, $(LIBRARIES))
