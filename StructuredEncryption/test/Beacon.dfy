...
module TestBaseBeacon {
  ...
  method {:test} TestBeacon() {
    ...
    b := Beacon(client := primitives, name := "foo", key := [1,2], length := 4, prefix := Some('-'));
    bytes :- expect b.getHmacBytes(UTF8.EncodeAscii("junk"));
    expect bytes[7] == 0x43;
    bytes :- expect b.getHmacBytes(UTF8.EncodeAscii("junk-foo"));
    expect bytes[7] == 0xb9;

    TestCompound(b, "stuff-junk", "stuff-3");
    TestCompound(b, "stuff-junk-foo", "stuff-9");
    TestCompound(b, "-junk", "-3");
    TestCompound(b, "_-junk", "_-3");
