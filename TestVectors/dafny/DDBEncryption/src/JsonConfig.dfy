...
module {:options "-functionSyntax:4"} JsonConfig {
  ...
  */
  ...
  method GetOneItemConfig(name : string, data : JSON) returns (output : Result<ENC.DynamoDbItemEncryptorConfig, string>)
    ensures output.Success? ==>
      && output.value.cmm.None?
      && output.value.keyring.Some? && output.value.keyring.value.ValidState() && fresh(output.value.keyring.value)
      && output.value.legacyOverride.None?
