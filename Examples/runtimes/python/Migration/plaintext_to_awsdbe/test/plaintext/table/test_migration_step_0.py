...
This test validates the initial plaintext state for DynamoDB operations with Table interface.
It verifies basic functionality of the plaintext DynamoDB Table operations.
"""
import pytest
...
def test_migration_step_0_with_table():
    """Test plaintext DynamoDB Table Resource operations."""
    ...
    # Then: Success (i.e. can read values in plaintext values)
    migration_step_0.migration_step_0_with_table(ddb_table_name=TEST_DDB_TABLE_NAME, sort_read_value=1)
    ...
    with pytest.raises(AssertionError):
        migration_step_0.migration_step_0_with_table(ddb_table_name=TEST_DDB_TABLE_NAME, sort_read_value=2)
    ...
    # AssertionError: attribute1 now has type (binary) value instead of (string) value
    with pytest.raises(AssertionError):
        migration_step_0.migration_step_0_with_table(ddb_table_name=TEST_DDB_TABLE_NAME, sort_read_value=3)
