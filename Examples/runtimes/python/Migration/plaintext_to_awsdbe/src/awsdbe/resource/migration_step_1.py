...
In this example, we configure an EncryptedResource to do the following:
...
While this step configures your resource to be ready to start reading encrypted items,
we do not yet expect to be reading any encrypted items,
as our resource still writes plaintext items.
...
def migration_step_1_with_resource(kms_key_id: str, ddb_table_name: str, sort_read_value: int = 1):
    ...
    #    These items will be stored in plaintext since we are using a plaintext override
