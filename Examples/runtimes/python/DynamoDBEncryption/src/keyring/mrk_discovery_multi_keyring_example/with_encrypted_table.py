...
"""
Example demonstrating DynamoDb Encryption using a MRK discovery multi-keyring with EncryptedTable.
...
def multi_mrk_discovery_keyring_table_example(
    ddb_table_name: str, key_arn: str, account_ids: List[str], regions: List[str]
):
    """
    Demonstrate using a MRK discovery multi-keyring with EncryptedTable.
    ...
    # 6. Put an item into our table using the above table.
    #    Before the item gets sent to DynamoDb, it will be encrypted
    #    client-side using the MRK multi-keyring.
    item = {
        "partition_key": "awsKmsMrkDiscoveryMultiKeyringItem",
        "sort_key": 0,
    ...
    encrypted_ddb_table_for_decrypt = EncryptedTable(table=ddb_table, encryption_config=tables_config_for_decrypt)

    # 10. Get the item back from our table using the above table.
    #     The table will retrieve encrypted items from the DDB table, then
    #     detect the KMS key that was used to encrypt their data keys.
    #     The table will make a request to KMS to decrypt with the encrypting KMS key.
    #     If the table has permission to decrypt with the KMS key,
    #     the table will decrypt the item client-side using the keyring
    #     and return the original item.
    key_to_get = {"partition_key": "awsKmsMrkDiscoveryMultiKeyringItem", "sort_key": 0}

    get_response = encrypted_ddb_table_for_decrypt.get_item(Key=key_to_get)
