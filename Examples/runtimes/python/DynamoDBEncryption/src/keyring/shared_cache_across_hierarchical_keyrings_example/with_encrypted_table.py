...
def get_ddb_table(
    ...
    return encrypted_ddb_table


def put_get_items(ddb_table: boto3.resource):
    """
    Put and get items using the given DynamoDB client.

    :param ddb_table: The DynamoDB client to use
    """
    ...
    # information.
    item = {"partition_key": "id", "sort_key": 0, "sensitive_data": "encrypt and sign me!"}
    ...
    # BranchKeyIdSupplier as per your use-case. See the HierarchicalKeyringsExample.java for more
    # information.
    key_to_get = {"partition_key": "id", "sort_key": 0}

    get_response = ddb_table.get_item(Key=key_to_get)
...
def shared_cache_across_hierarchical_keyrings_example(
    ...
    ddb_table1 = get_ddb_table(ddb_table_name, hierarchical_keyring1, attribute_actions_on_encrypt)

    # 7. Encrypt Decrypt roundtrip with ddb_table1
    put_get_items(ddb_table1)
    ...
    ddb_table2 = get_ddb_table(ddb_table_name, hierarchical_keyring2, attribute_actions_on_encrypt)

    # 11. Encrypt Decrypt roundtrip with ddb_table2
    put_get_items(ddb_table2)
