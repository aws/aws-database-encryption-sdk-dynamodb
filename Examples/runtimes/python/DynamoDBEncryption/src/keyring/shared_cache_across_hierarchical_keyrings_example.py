...
def get_ddb_client(
    ddb_table_name: str,
    hierarchical_keyring: IKeyring,
    attribute_actions_on_encrypt: Dict[str, CryptoAction]
) -> boto3.client:
    """
...
def put_get_items(ddb_table_name: str, ddb_client: boto3.client):
    ...
    put_response = ddb_client.put_item(
    ...
    get_response = ddb_client.get_item(
        TableName=ddb_table_name,
        Key=key_to_get
    )
...
def shared_cache_across_hierarchical_keyrings_example(
    ...
    cache = CacheTypeDefault(
        DefaultCache(entry_capacity=100)
    )
    ...
    hierarchical_keyring1 = mat_prov.create_aws_kms_hierarchical_keyring(
        input=keyring_input1
    )
    ...
    ddb_client1 = get_ddb_client(
        ddb_table_name,
        hierarchical_keyring1,
        attribute_actions_on_encrypt
    )
    ...
    hierarchical_keyring2 = mat_prov.create_aws_kms_hierarchical_keyring(
        input=keyring_input2
    )

    # 10. Get the DDB Client for Hierarchical Keyring 2.
    ddb_client2 = get_ddb_client(
        ddb_table_name,
        hierarchical_keyring2,
        attribute_actions_on_encrypt
    )
