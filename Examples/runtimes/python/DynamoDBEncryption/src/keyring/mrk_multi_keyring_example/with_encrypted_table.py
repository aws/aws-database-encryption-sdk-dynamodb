...
def multi_mrk_keyring_table_example(ddb_table_name: str, mrk_key_arn: str, key_arn: str, mrk_replica_key_arn: str):
    """
    Demonstrate using a MRK multi-keyring with EncryptedTable.
    ...
    # 6. Put an item into our table using the encrypted table.
    ...
    item = {
        "partition_key": "awsKmsMrkMultiKeyringItem",
        "sort_key": 0,
    ...
    # 7. Get the item back from our table using the encrypted table.
    ...
    key_to_get = {"partition_key": "awsKmsMrkMultiKeyringItem", "sort_key": 0}
    ...
    only_replica_key_encrypted_table = EncryptedTable(
        table=ddb_table,
        encryption_config=only_replica_key_tables_config,
    )

    # 10. Get the item back from our table using the table configured with the replica.
    #    The client will decrypt the item client-side using the replica MRK
    #    and return back the original item.
    only_replica_key_get_response = only_replica_key_encrypted_table.get_item(Key=key_to_get)
    ...
    assert only_replica_key_returned_item["sensitive_data"] == "encrypt and sign me!"
    ...
    only_srk_encrypted_table = EncryptedTable(
        table=ddb_table,
        encryption_config=only_srk_tables_config,
    )

    # 13. Get the item back from our table using the table configured with the single-region key.
    #     The client will decrypt the item client-side using the single-region key
    #     and return back the original item.
    only_srk_get_response = only_srk_encrypted_table.get_item(Key=key_to_get)

    # Demonstrate that GetItem succeeded and returned the decrypted item
    assert only_srk_get_response["ResponseMetadata"]["HTTPStatusCode"] == 200
    only_srk_returned_item = only_srk_get_response["Item"]
    assert only_srk_returned_item["sensitive_data"] == "encrypt and sign me!"
