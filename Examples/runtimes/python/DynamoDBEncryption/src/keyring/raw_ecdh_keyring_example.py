...
def raw_ecdh_keyring_get_item_put_item(
    ddb_table_name: str,
    curve_spec: str
):
    """
    ...
    try:
        ...
        raise RuntimeError("IOError while reading the private key from file") from e
    ...
    try:
        ...
        raise RuntimeError("IOError while reading the public key from file") from e
    ...
    raw_ecdh_keyring = mat_prov.create_raw_ecdh_keyring(
        input=keyring_input
    )
...
def ephemeral_raw_ecdh_keyring_put_item(
    ddb_table_name: str,
    curve_spec: str
):
    """
...
def discovery_raw_ecdh_keyring_get_item(
    ddb_table_name: str,
    curve_spec: str
):
    """
    ...
    keyring_input = CreateRawEcdhKeyringInput(
        ...
        key_agreement_scheme=RawEcdhStaticConfigurationsPublicKeyDiscovery(
            PublicKeyDiscoveryInput(
                recipient_static_private_key=private_key_utf8_encoded
            )
        )
    )
...
def put_get_example_with_keyring(
    raw_ecdh_keyring: IKeyring,
    ddb_table_name: str
):
    """
...
def generate_ecc_key_pairs():
    ...
    if (
        private_key_file_sender.exists()
        or public_key_file_recipient.exists()
        or private_key_file_recipient.exists()
    ):
        raise FileExistsError("generateEccKeyPairs will not overwrite existing PEM files")

    # Generate sender key pair
    sender_private_key = ec.generate_private_key(ec.ECC_NIST_P256())
    sender_public_key = sender_private_key.public_key()

    # Generate recipient key pair
    recipient_private_key = ec.generate_private_key(ec.ECC_NIST_P256())
