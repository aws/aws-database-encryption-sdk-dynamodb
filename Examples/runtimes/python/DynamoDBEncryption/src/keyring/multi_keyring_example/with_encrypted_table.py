...
def multi_keyring_table_example(ddb_table_name: str, key_arn: str, aes_key_bytes: bytes):
    """
    ...
    # 8. Put an item into our table using the above table.
    ...
    item = {
        "partition_key": "multiKeyringItem",
        "sort_key": 0,
    ...
    # 9. Get the item back from our table using the above table.
    #    The table will decrypt the item client-side using the AWS KMS
    ...
    key_to_get = {"partition_key": "multiKeyringItem", "sort_key": 0}
    ...
    only_aes_keyring_encrypted_table = EncryptedTable(table=ddb_table, encryption_config=only_aes_keyring_tables_config)

    # 11. Get the item back from our table using the client
    #     configured with only the raw AES keyring.
    #     The table will decrypt the item client-side using the raw
    #     AES keyring, and return back the original item.
    only_aes_keyring_get_response = only_aes_keyring_encrypted_table.get_item(Key=key_to_get)
    ...
    assert only_aes_keyring_returned_item["sensitive_data"] == "encrypt and sign me!"
