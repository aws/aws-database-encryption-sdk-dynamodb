...
"""
Example demonstrating DynamoDB encryption using beacon styles with EncryptedTable.
...
def beacon_styles_table_example(
    ddb_table_name: str,
    branch_key_id: str,
    ...
    """
    Demonstrate using different beacon styles with EncryptedTable.
    ...
    item2 = {
        ...
        "fruit": "orange",
        "dessert": "orange",
        "basket": {"blackberry", "blueberry", "strawberry"},
        "veggies": {"beans", "carrots", "peas"},
        "work_type": "large",
    }

    # 5. Add the two items
    put_response = encrypted_ddb_table.put_item(Item=item1)
    # Validate object put successfully
    assert put_response["ResponseMetadata"]["HTTPStatusCode"] == 200

    put_response = encrypted_ddb_table.put_item(Item=item2)
    # Validate object put successfully
    assert put_response["ResponseMetadata"]["HTTPStatusCode"] == 200

    # 6. Test the first type of Set operation:
    # Select records where the basket attribute holds a particular value
    expression_attribute_values = {":value": "banana"}

    scan_response = encrypted_ddb_table.scan(
    ...
    scan_response = encrypted_ddb_table.scan(FilterExpression="contains(basket, fruit)")
    # Validate query was returned successfully
    assert scan_response["ResponseMetadata"]["HTTPStatusCode"] == 200
    ...
    expression_attribute_values = {":value": {"boysenberry", "grape", "orange"}}
    ...
    scan_response = encrypted_ddb_table.scan(FilterExpression="dessert = fruit")
    # Validate query was returned successfully
    assert scan_response["ResponseMetadata"]["HTTPStatusCode"] == 200
    ...
    expression_attribute_values = {":value": "peas"}
    ...
    expression_attribute_values = {":value": "I-1.T-small"}
