...
def virtual_beacon_table_example(
    ddb_table_name: str,
    branch_key_id: str,
    ...
    """
    Demonstrate using virtual beacon searchable encryption with EncryptedTable.
    ...
    """
    # 1. Create the DynamoDb Encryption configuration using the setup_beacon_config function
    ...
    # 5. Put two items into our table using the above encrypted table.
    ...
    put_response = encrypted_table.put_item(Item=item_with_has_test_result)
    # Assert PutItem was successful
    assert put_response["ResponseMetadata"]["HTTPStatusCode"] == 200

    put_response = encrypted_table.put_item(Item=item_with_no_has_test_result)
    # Assert PutItem was successful
    assert put_response["ResponseMetadata"]["HTTPStatusCode"] == 200
    ...
    expression_attribute_values = {":stateAndHasTestResult": "CAt"}

    # GSIs do not update instantly
    # so if the results come back empty
    # we retry after a short sleep
    for _ in range(10):
        query_response = encrypted_table.query(
            IndexName=GSI_NAME,
        ...
        assert returned_item["state"] == "CA"
        assert returned_item["hasTestResult"] is True
        break
