...
"""
Example demonstrating DynamoDB encryption using compound beacons with EncryptedTable.
...
def compound_beacon_table_example(
    ddb_table_name: str,
    branch_key_id: str,
    ...
    """
    Demonstrate using compound beacon searchable encryption with EncryptedTable.
    ...
    encrypted_table = EncryptedTable(table=ddb_table, encryption_config=tables_config)

    # 3. Create an item with both attributes used in the compound beacon.
    item = {
        "work_id": "9ce39272-8068-4efd-a211-cd162ad65d4c",
        "inspection_date": "2023-06-13",
        "inspector_id_last4": "5678",
        "unit": "011899988199",
    }
    ...
    put_and_query_item_with_compound_beacon(encrypted_table, item)


def put_and_query_item_with_compound_beacon(ddb_table: EncryptedTable, item: Dict):
    """
    Put and query an item using a compound beacon.

    :param ddb_table: The encrypted DynamoDB client
    :param item: The item to put and query
    """
    # Write the item to the table
    put_response = ddb_table.put_item(Item=item)
    ...
    expression_attribute_values = {":value": "L-5678.U-011899988199"}
    ...
    for _ in range(10):
        ...
        assert returned_item["inspector_id_last4"] == "5678"
        assert returned_item["unit"] == "011899988199"
        break
